...
using Xunit;
using static TensionDev.Maritime.NMEA0183.DTMSentence;
...
namespace TensionDev.Maritime.NMEA0183.Tests
{
    public class UnitTestDTMSentence
    {
        ...
        [Theory]
        [InlineData(EarthCentredDatumEnum.WGS84, "W84")]
        [InlineData(EarthCentredDatumEnum.WGS72, "W72")]
        [InlineData(EarthCentredDatumEnum.SGS85, "S85")]
        [InlineData(EarthCentredDatumEnum.PE90, "P90")]
        [InlineData(EarthCentredDatumEnum.UserDefined, "999")]
        public void DTMLocalDatumEarthCentred(EarthCentredDatumEnum expectedEarthCentredDatumEnum, string expectedLocalDatumCode)
        {
            DTMSentence dtmSentence = new DTMSentence();
            dtmSentence.LocalDatumEarthCentred = expectedEarthCentredDatumEnum;

            var actualEarthCentredDatumEnum = dtmSentence.LocalDatumEarthCentred;
            var actualLocalDatumCode = dtmSentence.LocalDatumCode;

            Assert.Equal(expectedEarthCentredDatumEnum, actualEarthCentredDatumEnum);
            Assert.Equal(expectedLocalDatumCode, actualLocalDatumCode);
        }
    }
}
